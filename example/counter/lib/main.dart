...
/// 2. Try voice commands like "Hey Siri, increment counter with counter_example"
...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      title: 'Counter App Intents Example',
      theme: ThemeData(
        colorScheme: ColorScheme.fromSeed(seedColor: Colors.deepPurple),
        useMaterial3: true,
      ),
      home: const MyHomePage(title: 'Counter App Intents Example'),
    );
  }
}
  ...
  Future<void> _setupIntents() async {
    try {
      ...
      try {
        ...
        if (Platform.isIOS) {
          // iOS: Update system shortcuts and get registered intents
          await _client.updateShortcuts();
          _registeredIntents = await _client.getRegisteredIntents();
        }
      } on UnsupportedError catch (_) {
        // Android: Platform registration not supported, but handlers are set up
        // Intents are defined in shortcuts.xml (generated by CLI)
        // and will be routed via deep links to the registered handlers
      }

      // For Android, set up the registered intents list manually
